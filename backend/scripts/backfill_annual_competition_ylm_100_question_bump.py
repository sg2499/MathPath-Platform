...
YLM-L1) 100-question bump (2026-09-22, Shailesh): "we need to bump the
total number of question in both these papers to 100 as 50 is too less and
the students are easily able to complete all 50 of them even before the 10
min timer gets exhausted ... also we need to backfill the papers for these
2 levels the ones that have been assigned but not yet attempted and the
ones completed stay as is."

Root cause / forward-fix context: annual_competition_paper_registry.py's
ANNUAL_COMPETITION_LEVEL_REGISTRY["YLM-L0"]/["YLM-L1"] used to set SEC1's
questionCount to 50; both now read 100 (timeLimitSeconds left unchanged at
600 -- Shailesh's own framing was "make the existing 10-minute window
actually challenging," not lengthen it). Rules, guidelines and the concept
pool (_YLM_L1_DIRECT_POOL's pure single-digit / mixed / pure double-digit
direct add/less split) are untouched -- the even split across those three
concepts simply rescales from ~17/17/16 (of 50) to ~34/33/33 (of 100).
Uniqueness-within-a-paper is unaffected: _CollectAnnualCompetitionQuestions
(annual_competition_paper_generation_service.py) already tracks used
question signatures across the WHOLE paper and hard-fails loudly rather
than ever repeating one -- confirmed empirically safe at 100 questions via
30 live dry-run generations (15 per level, distinct seeds each) against
this exact registry before the registry change shipped.
GenerateAnnualCompetitionLevelPaper already reads that registry fresh on
every call, so every NEW paper generated from this point forward
automatically gets 100 questions -- but a practice paper's content is
generated ONCE and frozen as its own CompetitionMockExam/
CompetitionMockQuestion rows the moment it's assigned to a student (see
annual_competition_studio_service.py's _GeneratePracticePapersForOneStudent).
A paper that was already assigned before this change landed keeps its old,
50-question content forever unless something regenerates it -- that is
what this script does.

This mirrors backfill_annual_competition_ylm_direct_add_less_digit_mix.py's
own shape almost exactly (same underlying situation: a registry content
...
gate this codebase already uses everywhere else:
  - A PRACTICE CompetitionEventLevelPaper (paper_kind == "PRACTICE") for
    ...
    excluded too, never silently blown away. This is exactly what Shailesh
    asked for: "the ones completed stay as is."
  - OFFICIAL papers are reported (counted, listed) but NEVER touched by
    --apply -- same reasoning as the sibling digit-mix script: an official
    paper is shared across every student sitting that level, is a
    materially higher-risk single point of regeneration, and Shailesh's
    request named "the practice papers" specifically. Any OFFICIAL paper
    generated fresh from this point forward (including on the actual event
    day) already gets 100 questions automatically, with no backfill needed,
    since GenerateAnnualCompetitionLevelPaper reads the same registry for
    both official and practice generation.

Idempotent: a paper this script has already regenerated is tagged with
"Q100BUMP" in its new exam's mock_code (e.g.
ANNUAL-PRACTICE-YLM-L1-Q100BUMP-A1B2C3D4), and a paper whose CURRENT
...
Safe by design (identical guarantees to the sibling digit-mix script):
  ...
  - Regenerated content goes through the exact same GenerateAnnualCompetition
    ...
    already uses -- never a bespoke reimplementation, so it picks up
    whatever questionCount is live in the registry at run time (100, once
    the registry change is deployed -- this script does not hardcode 100
    anywhere itself).
...
AFTER the registry's questionCount:100 change has been deployed -- running
this before deploy would just regenerate papers back at the old 50 count):
    python scripts/backfill_annual_competition_ylm_100_question_bump.py --dry-run
    python scripts/backfill_annual_competition_ylm_100_question_bump.py --apply
...
# The exact levels whose questionCount just went 50 -> 100
# (annual_competition_paper_registry.py, 2026-09-22). Both share the same
...
def _is_already_backfilled(db, mock_exam_id: str | None) -> bool:
    ...
    return bool(ExamRecord and Q100_TAG in (ExamRecord.mock_code or ""))
    ...
    for paper in candidate_papers:
        ...
        try:
            exam_payload = GenerateAnnualCompetitionLevelPaper(
                ...
                CreatedBy=None,
                Title=f"Annual Competition Practice -- {paper.competition_level_code} (100-question bump backfill, 2026-09-22)",
                MockCode=f"ANNUAL-PRACTICE-{paper.competition_level_code}-{Q100_TAG}-{uuid4().hex[:8].upper()}",
...
def main() -> None:
    ...
    print("=" * 88)
    print(f"ANNUAL COMPETITION YLM (BLOOMERS/BEGINNERS) 100-QUESTION BUMP BACKFILL -- mode: "
