...
def _GenerateSingleSectionQuestionSet(Config: MMConfig, SectionNumber: int = 1, SectionTitle: str | None = None, StartNumber: int = 1, TotalSections: int = 1) -> list[dict]:
    ...
    for SectionQuestionNumber in range(1, QuestionCount + 1):
        ...
        OptionValues = {str(Option.get("value")) for Option in Options if isinstance(Option, dict)}
        if str(CorrectDisplay) not in OptionValues:
            raise ValueError(f"Correct answer option missing for {Config.ConceptFamily} question {SectionQuestionNumber}")
