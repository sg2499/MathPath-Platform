...
from app.question_engine.pm_l3.operands import generate_unique_operands, question_difficulty_stage
from app.question_engine.pm_l3.validators import question_concept_trace, validate_question
from app.question_engine.pm_l3.distractors import generate_distractors
from app.question_engine.pm_l3.concept_drill import generate_concept_drill_question
from app.question_engine.pm_l3.multiply import generate_multiply_table_question
from app.question_engine.pm_l3.divide import generate_divide_table_question
from app.question_engine.pm_l3.bodmas import generate_bodmas_question


def generate_pm_l3_question_set(config: PML3Config) -> list[dict]:
    """PM-L3's own, fully independent vertical Add/Less question generator.
    Zero imports from question_engine/pm, question_engine/pm_l2, or any
    other module's engine -- see this package's __init__.py docstring.
    """
    questions: list[dict] = []
    ...
    for question_number in range(1, config.question_count + 1):
        ...
        if not validate_question(effective_config, operands):
            raise ValueError(f"Generated invalid PM-L3 question for lesson {config.lesson_number} DPS {config.dps_number}")
...
def generate_pm_l3_bodmas_set(config: PML3BodmasConfig, count: int) -> list[dict]:
    questions: list[dict] = []
    for i in range(1, count + 1):
        q_rng = random.Random(f"{config.seed}-Q{i}")
...
def generate_pm_l3_concept_drill_set(specs: list[tuple[PML3ConceptDrillConfig, int]]) -> list[dict]:
    ...
    -- same sequential-numbering convention PM-L2's equivalent uses.
    """
    questions: list[dict] = []
    question_number = 1
