...
def generate_distractors(correct_answer: int, operands: list[int], rng: random.Random, allow_negative: bool = False) -> list[int]:
    ...
    enforce_same_digit_count=True (matches PM-L1/L2/L3): PM is a
...
def generate_multiply_table_distractors(correct_answer: int, rng: random.Random) -> list[int]:
    """PM-L4's standalone "2D X 1D" DPS -- same MULTIPLY strategy as Concept
...
def generate_bodmas_distractors(correct_answer: int, rng: random.Random) -> list[int]:
    """PM-L4's BODMAS expressions -- same as PM-L3's: no operand-specific
...
def generate_divide_table_distractors(number: int, divisor: int, correct_answer: int, rng: random.Random) -> list[int]:
    """PM-L4's standalone exact-division DPS (2D÷1D or 3D÷1D) -- real exact
...
def generate_divide_remainder_distractors(
    ...
    """PM-L4's new "3D ÷ 1D WITH REMAINDER(S)" concept -- no existing
    smart-distractor strategy models a paired quotient+remainder value, so
    this builds plausible wrong PAIRS directly: a wrong quotient (off by
    +/-1 or +/-2, the same near-miss magnitude the shared MULTIPLY/DIVIDE
    strategies use) paired with the correct remainder, the correct quotient
    paired with a wrong remainder (still kept in [0, divisor) so it still
    LOOKS like a valid remainder), and one pair with both wrong -- covering
    the three realistic mistake shapes (wrong quotient only, wrong
    remainder only, both wrong) rather than random unrelated numbers.
    Every distractor is formatted identically to the correct answer
    ("Q, R") and guaranteed distinct from it and from each other.
    """
