...
def _merge_section_config(lesson_number: int, dps_number: int, lesson_title: str, blocks: tuple[PmL3DpsBlock, ...]) -> dict:
    ...
    shape. Mirrors PM-L4's identical fix (2026-08-06).
    """
    return {
        "lessonNumber": lesson_number,
        "dpsNumber": dps_number,
...
def _ensure_section(db: Session, dps: DPS, lesson_number: int, dps_number: int, lesson_title: str, blocks: tuple[PmL3DpsBlock, ...], section_number: int) -> None:
    """Creates/updates one DPSSection from a group of 1+ PmL3DpsBlock
    ...
    instructions page (both read DPSSection rows directly). Fixed 2026-08-06
    after Shailesh reported this exact symptom live in Learning Path Studio
    for PM-L4 and asked for a full-level audit; this bug turned out to be
    identical and pre-existing in PM-L3 (already live in production at the
    time of this fix) -- seed()'s caller groups consecutive same-titled
    blocks before calling this function; a normal solo block (e.g. Add/Less)
    is just a 1-tuple here.
    """
    primary = blocks[0]
...
def seed(db: Session):
    ...
    for lesson_number in sorted(PM_L3_LESSONS):
        ...
        for dps_number in sorted(dps_rules):
            ...
            # one DPSSection -- see _ensure_section's docstring for why.
            groups: list[tuple[PmL3DpsBlock, ...]] = []
            ...
            # instructions page would keep showing the old split. This is
            # exactly what production needs on next deploy, since PM-L3 is
            # already live with the old 3-section layout.
