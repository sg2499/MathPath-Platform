...
from app.seed.preparatory_module_l2_config import PM_L2_LESSONS, PmL2DpsRule, PmL2ConceptDrillItem, pm_l2_dps_rows_for
...
def _ensure_normal_section(db: Session, dps: DPS, lesson_number: int, dps_number: int, lesson_title: str, rule: PmL2DpsRule, section_number: int) -> None:
    ...
    # 2026-09-02 -- narrow-pool DPS row override (see PM_L2_DPS_ROWS_OVERRIDES
    # in preparatory_module_l2_config.py): most DPS use the curriculum's
    # native rule.rows unchanged, but Lesson 6's four single-digit-pattern
    ...
    effective_rows = pm_l2_dps_rows_for(lesson_number, dps_number, rule.rows)
