...
import { Float, Sparkles, Stars, Icosahedron, TorusKnot, OrbitControls } from '@react-three/drei';
import { X, Award, ChevronRight, Zap } from 'lucide-react';
...
function RankCore3D({ rank }: { rank: string }) {
  ...
  // Custom colors for R3F mesh materials
  const rankColors: Record<string, { color: string; emissive: string }> = {
    COPPER: { color: '#c2410c', emissive: '#7c2d12' },
    BRONZE: { color: '#d97706', emissive: '#b45309' },
    SILVER: { color: '#94a3b8', emissive: '#475569' },
    GOLD: { color: '#fbbf24', emissive: '#b45309' },
    PLATINUM: { color: '#06b6d4', emissive: '#155e75' },
    EMERALD: { color: '#10b981', emissive: '#064e3b' },
    DIAMOND: { color: '#a855f7', emissive: '#4f46e5' },
  ...
  return (
    <Float speed={2} rotationIntensity={1.5} floatIntensity={2}>
      ...
      <Sparkles count={80} scale={5} size={3} speed={0.4} color={color} />
    </Float>
  );
}
...
export function RankInspectionModal({ isOpen, onClose, currentXp, currentRankTier }: RankInspectionModalProps) {
  ...
  const rankColors: Record<string, string> = {
    COPPER: 'from-orange-600 to-red-900 border-orange-500/50 shadow-orange-500/20 text-orange-400',
    BRONZE: 'from-amber-500 to-amber-900 border-amber-500/50 shadow-amber-500/20 text-amber-400',
    SILVER: 'from-slate-400 to-slate-700 border-slate-400/50 shadow-slate-400/20 text-slate-300',
    GOLD: 'from-yellow-400 to-yellow-700 border-yellow-400/50 shadow-yellow-400/20 text-yellow-400',
    PLATINUM: 'from-cyan-400 to-cyan-800 border-cyan-400/50 shadow-cyan-400/20 text-cyan-400',
    EMERALD: 'from-emerald-400 to-emerald-800 border-emerald-400/50 shadow-emerald-400/20 text-emerald-400',
    DIAMOND: 'from-purple-400 to-indigo-800 border-purple-400/50 shadow-purple-400/20 text-purple-400',
    CHAMPION: 'from-rose-500 to-red-900 border-rose-500/50 shadow-rose-500/20 text-rose-400',
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl h-[85vh] bg-slate-950/80 border border-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row z-10"
          >
            
            {/* Close Button */}
            <button 
              onClick={onClose}
              className="absolute top-6 right-6 z-50 p-3 bg-slate-900/60 border border-slate-800 hover:border-slate-700 rounded-full text-slate-400 hover:text-white transition-all hover:scale-105"
            >
              <X className="w-5 h-5" />
            </button>

            {/* Left Side: 3D Holographic Display */}
            <div className="relative w-full md:w-1/2 h-1/2 md:h-full bg-radial-gradient-dark overflow-hidden flex items-center justify-center border-b md:border-b-0 md:border-r border-slate-800/80">
            ...
            <div className="w-full md:w-1/2 h-1/2 md:h-full p-8 md:p-12 flex flex-col justify-between overflow-y-auto bg-slate-950/50">
