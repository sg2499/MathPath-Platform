...
// pattern) -- EnvLevelMasterySprout (YLM-L2) and EnvLevelMasteryBlossom
// (YLM-L3) that used to live here were removed along with those levels.
...
export function BadgeInspectionModal({ badge, config, onClose, enableSound = true }: BadgeInspectionModalProps) {
  ...
  const getShapeStyles = (iconName: string) => {
    const shapes: Record<string, { clipPath: string, w: string, h: string }> = {
      ...
      // 2026-08-12: YLM collapsed from 3 levels down to a single YLM-L1
      // level (matching the BM/MM one-level-per-module pattern) -- the
      // LevelMasteryYlmL2*/LevelMasteryYlmL3* shape entries that used to
      // live here were removed along with those levels.
    };
