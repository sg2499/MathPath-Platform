...
// MM is flat 1 mark/question everywhere. PM, BM, and IM (and any future
// concept-weighted module) are 5 marks for Skill Stacker/Concept Drill and 1
// otherwise -- PM's and BM's own Concept Drill sections carry the same
// 5-mark weighting as IM's, they just label the module-level default as
// "Flat" here since most of their sections are 1 mark and only Concept Drill
// deviates (kept as a distinct label from IM's so the admin can tell PM/BM's
// Concept Drill is the only weighted section, not a whole different scheme).
// YLM (added 2026-08-11) joins MM's exact flat scheme -- always 100
// questions at 1 mark each, no Skill Stacker/Concept Drill-style weighted
// concept at all. Any future module not listed here keeps the original
// fixed-100-total auto-balanced scheme. See ResolvedAssessmentQuestionMark()
// in assessment_engine_service.py.
function marksModeLabel(moduleCode: string | null | undefined) {
  if (moduleCode === "MM" || moduleCode === "YLM") return "Flat (1 Each)";
  if (moduleCode === "PM" || moduleCode === "BM") return "Flat (1 Each, 5 For Concept Drill)";
    ...
    <article className="group relative overflow-hidden rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-2xl dark:border-slate-800 dark:bg-slate-950">
      ...
      <div className="relative z-10">
        ...
        <div className="mt-5 rounded-[24px] bg-slate-50 p-4 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-3"><p className="math-block-header"><BookOpenCheck size={14} />Distribution Preview</p><p className="text-xs font-black text-slate-500">{blueprintDistributionEntries(item).length} {item.distributionMode === "SECTION_WISE" ? "sections" : "lessons"}</p></div>
  ...
  return (
    <div className="space-y-3">
      ...
      <section className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <DetailMetric icon={<Calculator size={16} />} label="Marks/Question" value={marksModeLabel(item.moduleCode)} helper={item.moduleCode === "MM" || item.moduleCode === "YLM" ? "Every Question Is 1 Mark" : item.moduleCode === "PM" || item.moduleCode === "BM" ? "5 For Concept Drill, 1 Otherwise" : item.moduleCode === "IM" ? "5 For Skill Stacker/Concept Drill" : "Exact 100-Mark Total"} />
...
function MiniCount({ label, value }: { label: string; value: string | number }) { return <div className="min-w-[120px] rounded-2xl bg-slate-50 p-4 text-center dark:bg-slate-900"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">{label}</p><p className="mt-1 text-xl font-black text-slate-950 dark:text-white">{value}</p></div>; }
